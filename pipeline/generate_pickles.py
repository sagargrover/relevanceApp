...
# python generate_pickles.py test | prod
args = sys.argv


def generate_pickles(input_pipe, output_path):
    try:
    ...
    joblib.dump(classifier, output_path + config["pickle"]["classifier_model_file"])
    joblib.dump(count_vectorizer, output_path + config["pickle"]["preprocess_model_file"])
...
if __name__ == "__main__":
    if len(args) >= 2 and args[1] == 'prod':
        generate_pickles(InputPipeline(), config["resource_dir"])
    else:
        generate_pickles(InputPipeline(), config["resource_dir"]+'test/')
